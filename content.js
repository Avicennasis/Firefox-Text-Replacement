...
// - All replacement rules are stored locally in your Firefox browser sync storage.
// -----------------------------------------------------------------------------
...
function processNode(node) {
  // PERFORMANCE: Safety checks (extensionEnabled, ignoredTags, isEditable)
  // are now performed by the caller (TreeWalker or processElement) to avoid
  // redundant checks for every node.
...
function processElement(element) {
  ...
  if (element.nodeType === Node.TEXT_NODE) {
    // Check safety before processing (since we removed checks from processNode)
    if (ignoredTags.has(element.parentNode.tagName) || isEditable(element.parentNode)) {
      return;
    }
    processNode(element);
    return;
