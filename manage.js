...
// signed-in Firefox devices.
// -----------------------------------------------------------------------------

// -----------------------------------------------------------------------------
// FIREFOX STORAGE LIMITS
// These limits are enforced by Firefox, not by this extension.
...
function validateStorageQuota(wordMap) {
    ...
    // Check total storage limit (Firefox's limit, not ours!)
    if (estimatedSize > SYNC_QUOTA_BYTES) {
        const usedKB = (estimatedSize / 1024).toFixed(1);
        const maxKB = (SYNC_QUOTA_BYTES / 1024).toFixed(0);
        return `Storage full! You're using ${usedKB} KB of Firefox's ${maxKB} KB limit. Please remove some rules to free up space.`;
    }

    // Check per-item limit (less common, but can happen with very long replacement texts)
    if (estimatedSize > QUOTA_BYTES_PER_ITEM) {
        return `This rule is too large. Firefox limits individual storage items to 8 KB.`;
    }
...
function loadWordMap() {
    chrome.storage.sync.get('wordMap', (data) => {
        ...
        Object.keys(wordMap).forEach(originalText => {
            ...
            addRowToTable(originalText, data.replacement, data.caseSensitive, enabled, fragment);
        });
...
function exportRules() {
    chrome.storage.sync.get('wordMap', (data) => {
        ...
        // Example: "firefox-text-replacement-rules-2026-01-09.json"
        const dateStr = new Date().toISOString().split('T')[0]; // YYYY-MM-DD
        a.download = `firefox-text-replacement-rules-${dateStr}.json`;
